...
- **Involved TMForum entities**:
  ...
  - _AppliedCustomerBillingRate (ACBR)_ (TMF678). This resource represents a bill created during the billing process. The bill refers to a _product_ , and to a _periodCoverage_ (i.e., the time coverage period of the bill). The bill has been paid <ins>only</ins> if the attribute `isBilled` is set to `true` and the attribute `bill` (i.e., CustomerBillRef) is valorised. 

1 **How to retrieve all the bills (i.e., __AppliedCustomerBillingRate_) related to a Product for a period coverage**
  - To retrieve the list of the bills (i.e., ACBR) generated by the billing subsystem you can use the `GET` method provided by the **TMForum specification** with the **fields** and **filters** in the query string:
    
> GET /appliedCustomerBillingRate?fields=...&{filtering}

The following is an example of getting all *AppliedCustomerBillingRates* using only filters in the *query string* related to a specific **Product** and **Period Coverage**:

```
GET /appliedCustomerBillingRate?product.id=urn:ngsi-ld:product:f942601c-1902-4b09-a498-23d844d21972&periodCoverage.startDateTime.eq=2025-06-19T10:04:38.983Z&periodCoverage.endDateTime.eq=2025-06-21T10:04:38.983Z
```
> Please note that if you want to add the `type` attribute, then you have to use `rateType` (instead of *type*); i.e., add `rateType=pay-per-use` in the query string above.

2 **How to check if the bill has been paid**
  - For each Bill of the list retrieved at the previous point, check if the `isBilled` attribute is set to `true`. The amount paid is indicated in the attributes `taxExcludedAmount` and 'taxIncludedAmount' for the amount wihout and with taxies, respectively. 
