...
- **Involved TMForum entities**:
  ...
  - _AppliedCustomerBillingRate (ACBR)_ (TMF678). This resource represents a bill created during the billing process. The bill refers to a _product_ , and to a _periodCoverage_ (i.e., the time coverage period of the bill). The bill has been paid <ins>only</ins> if the attribute `isBilled` is set to `true`.
